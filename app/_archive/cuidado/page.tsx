...
const HERO =
  "https://images.unsplash.com/photo-1447752875215-b2761acb3c5d?w=2000&q=80&auto=format&fit=crop";
const AMBIENTE =
  "https://images.unsplash.com/photo-1490730141103-6cac27aaab94?w=1600&q=80&auto=format&fit=crop";
...
function Eyebrow({ children, light = false }: { children: string; light?: boolean }) {
  return (
    <span
      style={{ fontFamily: "var(--font-inter)", color: light ? T.sage : "#5d8a82" }}
...
export default function Cuidado() {
  ...
  return (
    <main style={{ background: T.ivory, color: T.ink, fontFamily: "var(--font-inter)" }}>
      {/* NAV */}
      <header
      ...
      >
        <div className="flex items-center gap-5">
          <Link href="/" style={{ fontFamily: "var(--font-inter)", color: scrolled ? T.teal : "rgba(255,255,255,.85)" }} className="text-xs uppercase tracking-[0.12em] transition-colors hover:opacity-70">← propostas</Link>
          <span className="flex items-center gap-2.5" style={{ opacity: scrolled ? 1 : 0, transition: "opacity .4s" }}>
            <Image src="/brand/cuidado-symbol.png" alt="" width={30} height={30} style={{ height: "auto" }} />
      ...
      <section ref={heroRef} className="relative grain flex min-h-screen items-center overflow-hidden">
        ...
        <motion.div style={{ opacity: fade }} className="relative mx-auto w-full max-w-5xl px-6 md:px-10">
          ...
          >
            <Image src="/brand/cuidado-symbol-light.png" alt="" width={64} height={64} className="mb-7 opacity-95" />
      ...
      <section className="px-6 py-28 md:px-10 md:py-36" style={{ background: T.deep, color: "rgba(255,255,255,.8)" }}>
        <div className="mx-auto max-w-2xl">
          ...
          <Reveal><p className="mt-7 text-[17px] leading-relaxed">{CONTEXT.intro}</p></Reveal>
          <Reveal>
          ...
          <Reveal><p className="text-[17px] leading-relaxed">{CONTEXT.outro}</p></Reveal>
        </div>
      </section>
      ...
      <section className="px-6 py-28 md:px-10 md:py-36">
        <div className="mx-auto max-w-2xl">
          ...
          <Reveal><p className="mt-7 text-[17px] leading-relaxed text-[#4a5a57]">{NASCIMENTO.p1}</p></Reveal>
          <Reveal><p className="mt-5 text-[17px] leading-relaxed text-[#4a5a57]">{NASCIMENTO.p2}</p></Reveal>
        </div>
      </section>
              ...
              <Reveal key={c.name} delay={i * 0.1}>
                <div className="glass h-full rounded-3xl p-8" style={{ background: "rgba(255,255,255,.42)", borderColor: "rgba(255,255,255,.6)" }}>
                  <div className="flex h-16 items-center">
                    <img src={c.logo} alt={c.name} className="max-h-16 w-auto max-w-[120px] object-contain" style={c.logo.endsWith(".svg") ? { filter: "invert(1) brightness(.2)" } : undefined} />
                  </div>
                  <h3 style={{ fontFamily: "var(--font-cormorant)" }} className="mt-5 text-xl" >{c.name}</h3>
                  <p className="mt-3 text-[15px] leading-relaxed text-[#4a5a57]">{c.text}</p>
                </div>
              </Reveal>
            ))}
          </div>
      ...
      <section className="px-6 py-28 md:px-10 md:py-36">
        <div className="mx-auto max-w-2xl">
          ...
          <Reveal><p className="mt-7 text-[17px] leading-relaxed text-[#4a5a57]">{OQUEE.intro}</p></Reveal>
          <Reveal>
            <div style={{ fontFamily: "var(--font-cormorant)" }} className="my-7 text-3xl italic" >
              {OQUEE.not.map((n) => <p key={n} style={{ color: T.sage }}>{n}</p>)}
            </div>
          </Reveal>
          <Reveal><p className="text-[17px] leading-relaxed text-[#4a5a57]">{OQUEE.is}</p></Reveal>
          <Reveal>
      ...
      <section className="relative grain overflow-hidden">
        ...
        <div className="relative mx-auto max-w-2xl px-6 py-32 md:px-10 md:py-44">
          ...
          <Reveal><p className="mt-9 text-[17px] leading-relaxed text-white/85">{DIFER.intro}</p></Reveal>
          <Reveal>
              ...
              <Reveal key={b.title} delay={i * 0.06}>
                <div className="grid grid-cols-[110px_1fr] gap-6 border-b py-7" style={{ borderColor: "rgba(35,77,87,.15)" }}>
                  ...
                  <div>
                    ...
                    <p className="mt-1 text-[15px] text-[#4a5a57]">{b.desc}</p>
                  </div>
                </div>
              </Reveal>
            ))}
            ...
            {[
              ...
              <Reveal key={m.k} delay={i * 0.08}>
                <div className="glass rounded-3xl p-9 text-center" style={{ background: "rgba(255,255,255,.5)", borderColor: "rgba(255,255,255,.7)" }}>
                  <div className="text-[11px] uppercase tracking-[0.2em] text-[#5d8a82]">{m.k}</div>
      ...
      <section className="px-6 py-28 md:px-10 md:py-36">
        <div className="mx-auto max-w-2xl">
          ...
          <Reveal><p className="mt-6 text-[17px] text-[#4a5a57]">{QUEM.intro}</p></Reveal>
          <ul className="mt-8 grid gap-3.5 sm:grid-cols-2">
            {QUEM.items.map((it, i) => (
              <Reveal as="li" key={it} delay={i * 0.05}>
                <span className="flex items-start gap-3 text-[17px] text-[#234d57]">
        ...
        <div className="mx-auto max-w-2xl">
          ...
          <Reveal>
            <div className="glass-dark mx-auto mt-14 max-w-lg rounded-[26px] p-10 text-white" style={{ background: "rgba(22,52,59,.92)" }}>
              <div className="text-[11px] uppercase tracking-[0.24em]" style={{ color: T.sage }}>Inscrição</div>
